...
public class SinTest {
    ...
    public void isPeriodic(double i, double expected) {
        assertEquals(expected, sin.compute(i * Math.PI / 4, eps), 1E-6);
        assertEquals(expected, sin.compute((i + 2) * Math.PI / 4, eps), 1E-6);
    }
    ...
    public void returnsCorrectResult(double x, double expected){
        assertEquals(expected, sin.compute(x, eps), eps*1000);
    }
}
